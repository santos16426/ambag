...
import { deleteSettlement } from "../services/transaction-submit.service";
import type { TransactionItemSettlement } from "../types";

interface DeleteSettlementModalProps {
  settlement: TransactionItemSettlement | null;
  onClose: () => void;
  onSuccess: (settlementId: string) => void;
}

export function DeleteSettlementModal({
  settlement,
  onClose,
  onSuccess,
}: DeleteSettlementModalProps) {
  ...
  async function handleConfirm() {
    ...
    setIsDeleting(true);
    setError(null);
    const { success, error: deleteError } = await deleteSettlement(settlement.id);
    setIsDeleting(false);
    if (deleteError || !success) {
      setError(deleteError?.message ?? "Failed to delete settlement. Please try again.");
      return;
    }
            ...
            <div className="px-6 pb-6 space-y-2">
              ...
              {settlement && (
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                  ₱
                  {settlement.amount.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </p>
              )}
              <p className="text-[11px] text-slate-400 font-medium pt-1">
                This action cannot be undone. Balances will be updated.
              </p>
            </div>
